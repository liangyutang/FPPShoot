...
void AAIGuard::OnNoiseHeard_Implementation(APawn* Instigat, const FVector& Location, float Volume)
{
...
/*void AAIGuard::OnNoiseHeard(APawn* Instigat, const FVector& Location, float Volume)
{
	...
	/*UE_LOG(LogTemp,Error,TEXT("2:%f  %f   %f"),Location.X,Location.Y,Location.Z);#1#
	//获得朝向
	FVector Direction=Location-GetActorLocation();
	Direction.Normalize();
